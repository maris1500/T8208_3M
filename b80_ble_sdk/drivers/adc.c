...
volatile unsigned short g_adc_vref = 1200;//1175;//ADC calibration value voltage (unit:mV).
_attribute_data_retention_
...
unsigned int adc_sample_and_get_result(void)
{
	...
	volatile signed int adc_data_buf[ADC_SAMPLE_NUM] = {0};  //size must 16 byte aligned(16/32/64...)
...
//  get adc sample data and sort these data 
	for(i = 0; i < ADC_SAMPLE_NUM; i++) 
	{
		...
		else
		{
			...
			//adc_vl_value = ((unsigned short)adc_data_buf[i] & 0x1FFF); 
		}
...
//  get average value from raw data(abandon some small and big data ), then filter with history data //////
	...
	#if (PROJECT_ID == PID_M705) || (PROJECT_ID == PID_BG523) || (PROJECT_ID == PID_NR300) || (PROJECT_ID == PID_TB903)
		// do nothing
	#elif (PROJECT_ID == PID_4027_DRY) && M4027_DRY_H2_ENABLE
		// do nothing
	#elif (PROJECT_ID == PID_MS631) || (PROJECT_ID == PID_MS358B)
		#if MS6351_CHARGE_ENABLE
			adc_vol_mv = ( adc_vol_mv << 1 );
		#else
			// for nothing
		#endif
	#else
